#!/usr/bin/env python3
"""Connector: FREE non-profit vision resources (KS + MO).

Loads the strictly-verified records from the `free-dental` workflow
(data/vision_records.json). The workflow only keeps resources confirmed FREE +
...
bucket = 'vision'. Honest by design: if the workflow found none for a zone,
...
RECORDS = os.path.join(ROOT, "data", "vision_records.json")
...
CENSUS_DELAY = 0.5
SOURCE_ID = "free-vision"
DIR_URL = "https://lionsclubs.org/"
TODAY = "2026-06-24"
...
def load(dry_run=False):
    if not os.path.exists(RECORDS):
        sys.exit(f"missing {RECORDS} — run the free-vision workflow first")
    ...
    if not dry_run:
        cur.execute("""INSERT OR REPLACE INTO source
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (SOURCE_ID, "Free non-profit vision resources (KS+MO)", "other", "various nonprofits",
             DIR_URL, 1, "org-public-facts", "Free/charitable dental resources — verify with each org", 0, 1,
             "open", "snapshot", TODAY, "medium",
             "Extracted + strictly verified (free + non-profit only (vision)) via free-vision workflow."))
    ...
    for r in recs:
        ...
        cur.execute("""INSERT OR REPLACE INTO service (id,organization_id,name,description,resource_bucket,status,source_id,source_url,date_checked,confidence)
                       VALUES (?,?,?,?,?,?,?,?,?,?)""",
                    (svc_id, org_id, "Free vision care / eyeglasses", what, "vision", "active", SOURCE_ID, src, TODAY, conf))
    ...
    if not dry_run:
        ...
        con.commit()
    con.close()
    print(f"[free-vision] sites={stats['sites']} programs={stats['programs']} "
